window.addEventListener('load', function () {
    ...
    if (btnSend && statusEl) {
        btnSend.addEventListener('click', function () {
            ...
            emailjs.send('service_39lwtcb', 'template_qwoxv1q', {
    ...
    if (typewriterElement) {
        const text = typewriterElement.getAttribute('aria-label') || '';
        typewriterElement.textContent = '';
        let index = 0;
        const typeSpeed = 50; // milliseconds per character
        const pauseAfterComplete = 2000; // pause before restarting
        ...
        function typeWriter() {
            if (index < text.length) {
                ...
                // After typing, pause then restart
                setTimeout(() => {
                    typewriterElement.textContent = '';
                    index = 0;
                    typeWriter();
